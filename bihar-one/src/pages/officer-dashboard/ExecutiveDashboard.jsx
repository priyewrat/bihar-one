...
function ExecutiveDashboard() {
  ...
  // Determines if a status means "Pending" on this dashboard
  const isPendingStatus = (status) => {
    if (!status) return true;
    const s = status.toUpperCase();
    return s.includes("ASSIGNED_TO_BO") || s === "ASSIGNED_TO_BO"; 
  };
  ...
  const handleSelectApplicant = async (app) => {
    try {
      ...
      formattedDetail.executiveRemarks = formattedDetail.executiveRemarks || app.executiveRemarks;
      
      setSelectedApplicant(formattedDetail);
      setRemarks(formattedDetail.executiveRemarks || "");
    } catch (error) {
      console.error("Error fetching application details:", error);
      setSelectedApplicant(app);
      setRemarks(app.executiveRemarks || "");
    }
  };
  ...
  const updateApplicationStatus = async (status, autoRemark = remarks) => {
    ...
    try {
      ...
      // 👇 This formats the data as x-www-form-urlencoded exactly like Postman! 👇
      const formData = new URLSearchParams();
      formData.append("remark", autoRemark);

      await axios.post(
        `${backendUrl}/residence/verify/bo/${selectedApplicant.id}`,
        formData,
        { 
          headers: { 
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/x-www-form-urlencoded" // Forces Axios to match Postman
          } 
        }
      );
      ...
      const updatedApplications = allData.map((app) =>
        app.id === selectedApplicant.id
          ? { ...app, status, executiveRemarks: autoRemark, boRemark: autoRemark }
          : app
      );
    ...
    // Converts the base64 string to a Blob and opens it in a new tab
  ...
  const processedApplications = allData.filter((app) => !isPendingStatus(app.status));
  ...
  const ApplicantListItem = ({ app }) => {
    ...
    // Automatically converts things like "VERIFIED_BY_BO" to "VERIFIED BY BO"
    ...
    <div className="min-h-screen bg-gray-100 p-6">
          ...
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1 md:w-72">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent text-sm"
              />
            </div>
            <button
              type="submit"
              className="bg-blue-700 hover:bg-blue-800 text-white px-5 py-2 rounded-lg font-medium shadow-sm transition-colors text-sm whitespace-nowrap"
            >
              Search
            </button>
          </form>
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-8">
        <div className="bg-blue-600 text-white rounded-xl p-6 flex items-center justify-between shadow-md">
          <div>
            <h3 className="text-lg font-medium text-blue-100">Total Applications</h3>
            <p className="text-4xl font-bold mt-1">{totalApplications}</p>
          </div>
        ...
        <div className="bg-blue-800 text-white rounded-xl p-6 flex items-center justify-between shadow-md">
          <div>
            <h3 className="text-lg font-medium text-blue-200">Processed</h3>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-md flex flex-col h-[700px] overflow-hidden">
          ...
          <div className="overflow-y-auto flex-1 bg-white">
            ...
            {pendingApplications.length === 0 ? (
              ...
              pendingApplications.map((app) => <ApplicantListItem key={app.id} app={app} />)
            )}
            ...
            {processedApplications.length === 0 ? (
              <div className="p-6 text-center text-gray-500">No processed applications</div>
            ) : (
              processedApplications.map((app) => <ApplicantListItem key={app.id} app={app} />)
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-md h-fit">
          <div className="bg-blue-700 text-white p-4 rounded-t-xl">
            <h2 className="font-semibold">Applicant Information</h2>
          </div>

          {selectedApplicant ? (
            <div className="p-6">
              {(selectedApplicant.fieldOfficerRemarks || selectedApplicant.roRemarks) && (
                <div className={`grid ${selectedApplicant.fieldOfficerRemarks && selectedApplicant.roRemarks ? "md:grid-cols-2" : "md:grid-cols-1"} gap-4 mb-6`}>
                  {selectedApplicant.fieldOfficerRemarks && (
                    <div className="bg-green-50 border border-green-200 rounded-xl p-4 shadow-sm">
                      ...
                      <p className="text-green-900 text-sm">{selectedApplicant.fieldOfficerRemarks}</p>
                    </div>
                  )}

                  {selectedApplicant.roRemarks && (
                    <div className={`border rounded-xl p-4 shadow-sm ${selectedApplicant.status.toUpperCase().includes("APPROVED") ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}>
                      <h3 className={`text-sm font-bold mb-2 flex items-center gap-2 ${selectedApplicant.status.toUpperCase().includes("APPROVED") ? "text-green-800" : "text-red-800"}`}>
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        RO Decision ({selectedApplicant.status.replace(/_/g, ' ')})
                      </h3>
                      <p className={`text-sm ${selectedApplicant.status.toUpperCase().includes("APPROVED") ? "text-green-900" : "text-red-900"}`}>
                        {selectedApplicant.roRemarks}
                      </p>
                    </div>
                  )}
              ...
              {/* Address Information */}
              <div className="bg-gray-50 border rounded-xl p-5 mb-6">
                <h3 className="text-lg font-bold text-blue-800 mb-4">Address Information</h3>
                <div className="grid md:grid-cols-2 gap-4">
              ...
              {/* Documents */}
              <div className="bg-gray-50 border rounded-xl p-5 mb-6">
                <h3 className="text-lg font-bold text-blue-800 mb-4">Uploaded Documents</h3>
              ...
              {/* Remarks */}
              <div className="mb-6">
                <label className="block font-semibold mb-2">Executive Remarks</label>
                <textarea
                  rows="4"
                  value={remarks}
                  onChange={(e) => setRemarks(e.target.value)}
                  disabled={isCompleted}
                  className={`w-full border rounded-lg p-3 outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent ${
                    isCompleted
              ...
              <div className="mt-8 border-t pt-6">
                <div className="flex gap-3 flex-wrap">
                  <button
                    disabled={isCompleted}
                    onClick={() => {
                      const finalRemark = remarks.trim() !== "" ? remarks : "Documents reviewed and forwarded for field verification.";
                      updateApplicationStatus("VERIFIED_BY_BO", finalRemark);
                    }}
                    className={`px-8 py-3 rounded-lg font-medium transition-colors shadow-sm ${
                      isCompleted
                        ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                        : "bg-blue-700 hover:bg-blue-800 text-white hover:shadow-md cursor-pointer"
                    }`}
                  >
                    {isCompleted ? "Forwarded" : "Forward To Field Officer"}
                  </button>
